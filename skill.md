...
description: LifeVault v0.1-v1.0.1 的版本迭代、实现路径与工程学习记录。
---

# LifeVault 版本迭代学习手册

这份文档记录 LifeVault 从 v0.1 到 v1.0.1 的演进过程。它不是产品使用说明，而是面向学习者的实现索引：每一版解决什么问题、为什么这样拆分、代码写在哪里、可以学到什么。
...
| v1.0.1 | `5b968d9` | 退货期限表达修复 | 将真实网页失败输入转成规则、单测和评测回归 |
...
## 24. v1.0.1：从网页失败输入建立回归

**问题**

网页演示输入使用了“14 天内可以退货”。原规则只接受“14 天退货”“14 天可退”和“14 天无理由”，因此模型候选虽然识别到两个提醒，却没有得到 `return_days`。确定性日期工具无法凭空计算退货截止日，Graph 正确停在 `missing_fields`，但常见自然语言没有被理解。

**修复**

`_extract_return_days()` 将“内/以内”与“可以/可/支持退货”作为受限可选表达，不放宽数字位置，也不从提醒提前天数猜测退货期限。用户的完整机械键盘输入同时进入单元测试和创建评测集，覆盖 `return_days=14`、两年保修以及两个独立提醒提前量。

修复后 fallback 创建评测从 72 条增加到 73 条，结果为 73/73 case、460/460 field。这个补丁说明线上或演示中的失败样例应先固化为可复现测试，再修改最小规则，最后运行完整评测，不能只在 UI 上手工试一次。

**学习重点**

- Graph 的缺字段中断是安全行为，但上游提取器仍应覆盖常见表达。
- 提醒提前 2 天与退货期限 14 天语义不同，不能互相推断。
- 用户原始输入是最有价值的回归样例，修复应同时进入窄单测和宽评测集。
